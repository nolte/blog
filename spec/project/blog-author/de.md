...
- Hinweis: die **Schwelle** zwischen Update und Neuanlage — wie groß muss eine sachliche Änderung sein, um statt eines Updates einen neuen Post zu rechtfertigen — ist bewusst **Autoren-Urteil** und nicht durch ein hartes Kriterium geregelt. Eine spätere Spec-Verschärfung wird durch einen konkreten Streitfall ausgelöst, nicht prospektiv.
...
Die hier verbleibenden offenen Fragen sind alle **blockiert durch die externe Spec `claude-shared/spec/project/lektorat/`**, die heute noch in `draft` steht. Solange `lektorat-apply` nicht released ist, würden Skill-Entwicklung, Vertrags-Wechsel und Schema-Formalisierung gegen einen noch nicht stabilisierten Vertragspartner laufen — deshalb sind sie nicht „aufgeschoben", sondern **wartend auf den externen Trigger**.

- **Operationalisierungs-Skill `blog-author`.** Diese Spec ist als Vertragsdokument für einen späteren `blog-author`-Skill in [`claude-shared`](https://github.com/nolte/claude-shared) angelegt. Der Skill würde die §Briefing-Inputs als Eingabe-Schema übernehmen, die §Workflow-Schritte als interaktive Phasen orchestrieren und den §Pre-Handoff-Selbst-Check als interne Verifikation implementieren. **Blockiert durch:** `claude-shared/spec/project/lektorat/` — ohne released `lektorat-apply` hätte der Skill keinen vollständigen Vertragspartner für Schritt 7 (Übergabe). Bis dahin bleibt der Workflow ein Mensch-plus-Claude-Code-Workflow gemäß `CLAUDE.md`.
- **Verdrahtung des Lektor-Übergabe-Vertrags.** §Übergabe an den Lektor adressiert zwei Zustände: das heutige Übergangsregime (Selbst-Lektorat plus optional `prose-vale-curator`) und den Zielzustand (Lauf von `lektorat-apply`). Der **Wechsel** zwischen den beiden Zuständen — wer markiert wann den Übergang als vollzogen, ob ein Repo-lokales Flag erforderlich ist, ob ein Spec-Edit den Übergang vollzieht — ist nicht definiert. Vermutlich genügt ein `Status:`-Versionswechsel in dieser Spec. **Blockiert durch:** `claude-shared/spec/project/lektorat/`, solange jene Spec ihren eigenen `status` aus `draft` nicht hebt.
- **Briefing und Liefervertrag als YAML-Schema.** §Briefing-Inputs und §Liefervertrag sind heute Prosa mit Pflichtfeld- bzw. Pflicht-Artefakt-Listen. Eine maschinen-lesbare Form für beide Seiten — `project/briefings/<slug>.yml` mit Schema-Validierung als **Eingang**, `project/handovers/<slug>.yml` (oder ein eingebettetes Frontmatter-Sub-Objekt) als **Ausgang** — wäre für einen späteren `blog-author`-Skill bequemer als Prosa und macht den Liefervertrag automatisch prüfbar statt nur attestierbar. **Blockiert durch:** die Operationalisierung des `blog-author`-Skills (transitiv über die `lektorat`-Spec). Bis dahin ist die Prosa-Form auf beiden Seiten ausreichend. Wenn die maschinen-lesbare Form eingeführt wird, sind die drei in §Liefervertrag genannten Artefakte (Selbst-Check-Manifest, Quellen-zu-Aussage-Mapping, Übergabe-Manifest) die natürlichen Top-Level-Schemata.

### In die Roadmap verschoben

Die folgenden Punkte sind **keine Spec-Fragen mehr**, sondern Produkt- bzw. Tooling-Entscheidungen; sie leben als Roadmap-Items in [`project/roadmap.md`](../../../project/roadmap.md):

- **Hero-Bild-Politik des Korpus.** Pflichtgrad, Korpus-Stil, Generierungs-Pipeline und Layout-Rendering des `heroImage`-Felds (heute nur als `og:image` im `<head>` gerendert, nicht visuell im Layout) werden in einem dedizierten Roadmap-Item zur Hero-Bild-Konzeption geführt. Die Spec hält weiterhin die Alt-Text-Regel und das Lede-Ersatz-DARF-NICHT in §Hero- und OG-Bild; die übergeordnete Politik wird dort entschieden.
- **Vale-Teilverdrahtung des Selbst-Checks.** Die maschinelle Verdrahtung des §Pre-Handoff-Selbst-Checks über `prose-vale-curator` (EN-Mechanik) wird in einem dedizierten Roadmap-Item zur Vale-Verdrahtung geführt. Die Spec hält weiterhin den Selbst-Check als Autoren-Pflicht und Reviewer-Urteil; die Tool-Verdrahtung wird dort entschieden. Eine **vollständige** maschinelle Verdrahtung der Per-Post-Akzeptanzkriterien aus den Geschwister-Specs bleibt an die oben gelistete Operationalisierungs-Frage und damit an den Lektor-Release gekoppelt.

### Bewusst nicht offen

- **Update-vs-Neuanlage-Schwelle** ist keine offene Frage, sondern eine bewusste Reaktiv-Entscheidung; die Schwelle ist Autoren-Urteil (siehe §Briefing-Inputs, Update- vs. Neuanlage-Felder). Ein konkreter Streitfall würde eine spätere Spec-Verschärfung auslösen.

...
- **A3** — is `/speckit.nolte-media.image-generate` ergonomically bearable next to today's `/nolte-media:image-generate`? That dotted form is the extension-command notation, not the `/speckit-plan` invocation form above — an extension namespaces its commands under the tool's own prefix.

...
- **A3** — ist `/speckit.nolte-media.image-generate` neben dem heutigen `/nolte-media:image-generate` ergonomisch zumutbar? Die Punktform ist die Notation für Extension-Befehle, nicht die oben beschriebene Aufrufform `/speckit-plan` — eine Extension hängt ihre Befehle unter das Präfix des Werkzeugs.
